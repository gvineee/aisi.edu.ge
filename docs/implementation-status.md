...
განახლებულია: 2026-09-11 · ეტაპი 1 დასრულებულია (ბრენდის სინქრონიზაცია + სრული საჯარო საიტი, ახლა დიზაინთან მჭიდრო fidelity-თი — იხ. ქვემოთ); **ეტაპი 2 მნიშვნელოვნად დაწინაურებულია** (როლზე დაფუძნებული dashboard-ები ყველა როლისთვის — guardian/teacher/student/director/admin —, სრული sidebar/bottom-nav portal layout, დოკუმენტების ცენტრის Stage 1+2, კონტენტის მიგრაციის importer ყველა 92 source key-ს ცხადი გადაწყვეტილებით). `CLAUDE-PLATFORM-MODULES.md`-ის 12 ახალი მოდულიდან **Messaging (Inbox) ახლა რეალურად მუშაობს** (იხ. ქვემოთ); დანარჩენი 11 (Daily Center, Portfolio, Assessment, Finance და ა.შ.) ჯერ არ დაწყებულა — დეტალები `docs/design-parity-checklist.md`-ში. სრული ტესტ-suite: **126/126 passed, 486 assertions** (Pint/PHPStan level 7/`npm run check`/`npm run types:check`/`npm run build` ყველა სუფთა).

...
### 2026-09-15 (გაგრძელება, მე-3) — Pickup & Consent (`app/Domain/PickupConsent`) production დეპლოი

`6cc4e87` (მოდული) + `ba30bb2` (fix: pickup-management ახლა გატესტილია `guardian_links.can_pickup`-ზე) რეალურად დეპლოირებულია იმავე დადგენილი ნიმუშით: `npm run build` → `git diff --name-only 48ffd32..HEAD -- app database resources routes` სიით 22 შეცვლილი ფაილი (3 ახალი migration, 3 Action, 3 Model, 2 Controller, 3 Form Request, 4 React გვერდი, `portal-layout.tsx`/`portal.ts` ტიპები, `routes/web.php`, `PortalContext.php`) + მთლიანი ახალი `public/build` → `scp` → `tar -xzf` → `clpctl system:permissions:reset --directories=770 --files=660 --path=.` → `storage/framework/views/*.php` წაშლა → `migrate --force` (3 ახალი migration, ყველა `DONE`) → `config:clear && config:cache`.

**იგივე ცნობილი route-cache ბაგი კვლავ განმეორდა** (დოკუმენტირებული 2026-09-14-ში, `route:clear && route:cache` მას შემდეგ ყოველ დეპლოის ნაბიჯშია ჩართული, მაგრამ ამ დავალების საწყისმა ინსტრუქციამ ეს ნაბიჯი არ ახსენა): `route:list`-მა თავიდან არცერთი `pickup.*`/`consents.*` route არ აჩვენა, თუმცა `routes/web.php`-ში სწორად იყო რეგისტრირებული — მიზეზი ისევ მოძველებული `bootstrap/cache/routes-v7.php` იყო (წინა, `admin:reset-password`-ის დეპლოიდან დარჩენილი). გასწორდა `route:clear && route:cache`-ით; ამის შემდეგ ყველა 7 route (`pickup.index/store/remove`, `consents.index/store/roster/respond`) გამოჩნდა.

დამოწმებულია production-ზე რეალურად: `route:list | grep -i pickup|consent` აჩვენებს ყველა 7 route-ს route-cache-ის გასწორების შემდეგ; `curl https://aisi.edu.ge/` → `200`; `curl https://aisi.edu.ge/portal/pickup` → `302` (არაავტორიზებული სტუმრისთვის, სწორი). `migrate:status` სამივე ახალ migration-ს `Ran`-ად აჩვენებს. დეპლოის tar.gz სერვერიდან და ლოკალურად წაშლილია. ლოკალურად: 10 ახალი ტესტი (`tests/Feature/PickupConsent/PickupConsentWorkflowTest.php`), commit-ის დროს უკვე "already-tested" იყო აღნიშნული — ამ სესიაში სუიტი ხელახლა არ გაშვებულა, მხოლოდ დეპლოი შესრულდა.

**ცნობილი დარჩენილი ხარვეზი (დოკუმენტაცია, არა ბლოკერი)**: `docs/design-parity-checklist.md`-ის "Safety (pickup/consent)" მწკრივი `not_started`-ად იყო მონიშნული `ba30bb2`-ის შემდეგაც — განახლდა ამ დეპლოისთანავე.

### 2026-09-15 (გაგრძელება, მე-4) — Pickup & Consent დამოუკიდებელი production ვერიფიკაცია: **კოდის ხარვეზი ვერ დადასტურდა; საწყისი "production" label გასწორდა**

ცალკე სესიაში მოთხოვნილი იყო "root cause"-ის გასწორება production-ვერიფიკაციისას ნაპოვნი პრობლემებისთვის: production-ზე (read-only SSH tinker) `users=1` (მხოლოდ seed-ის admin), `tenants=1`, `students=0`, `guardian_links=0`, `authorized_pickups=0`, `consent_forms=0`, `consent_responses=0` — ანუ არცერთი მშობელი/მოსწავლე არ არსებობს production-ში, guardian-only `pickup` nav-item-ის რენდერი და guardian-ის მხრიდან create→respond ციკლი ვერ დაიდასტურა ცოცხალ სესიაზე, და production admin-ის credential არ იყო ხელმისაწვდომი (`admin:reset-password`-ის გაშვება ცალკე ავტორიზაციას საჭიროებს — ცნობილი, არა ახალი, ბლოკერი, იხ. ქვემოთ Admissions-ის იდენტური ჩანაწერი).

**დამოუკიდებელი re-review-ის დასკვნა: არცერთი კოდის/migration/deploy-steps დეფექტი არ ნაპოვნია.** ხელახლა შემოწმდა `PickupController`/`ConsentController` (ორივე action ხელახლა ამოწმებს caller-ის საკუთარ აქტიურ `guardian_links` (+ `pickup`-ისთვის ცალკე `can_pickup` ბიტი) — client-supplied `student_id`-ს არასდროს ენდობა), სამივე migration (ინდექსების/foreign key-ების სახელები MySQL-ის 64-სიმბოლოიან ლიმიტს არ არღვევს — შემოწმებულია წინა substitution_assignments-ის მსგავსი კლასის ბაგისთვის, აქ ასეთი ხარვეზი არ არის), სამივე Form Request (`authorize()` განზრახ `true`-ია, კონტროლერი კვლავ ამოწმებს როლს/ბმულს — იგივე ნიმუში, რაც სხვა მოდულებში) და `PortalContext::PICKUP_ACCESS_ROLES`/`CONSENT_ACCESS_ROLES` (მხოლოდ `guardian` პირველისთვის; `guardian`+`director`+... მეორისთვის — კონტროლერის `STAFF_ROLES`-თან სინქრონში). **დასკვნა: ეს არის data/onboarding ხარვეზი (production-ს ჯერ არ ჰყავს რეალური მშობელი/მოსწავლე), არა კოდის ბაგი** — CLAUDE.md-ის აშკარა წესის გამო (production-ში ყალბი ბავშვის/მშობლის ჩანაწერის შექმნა აკრძალულია) ეს სესია ასეთ ჩანაწერს არ შექმნის და admin-ის production-პაროლსაც არ არეგულირებს მომხმარებლის კონკრეტული ავტორიზაციის გარეშე.

**რა შესრულდა ამის ნაცვლად, სრული ლოკალური quality gate-ის სახით:** `php artisan test --filter=PickupConsent` → 10/10 passed, 51 assertion; სრული სუიტი `php artisan test` → **238/238 passed, 1168 assertion** (რეგრესია არ დაფიქსირდა); `./vendor/bin/pint --test` → სუფთა; `./vendor/bin/phpstan analyse` (level 7) → 0 შეცდომა; `npm run types:check` (`tsc --noEmit`) → სუფთა. Guardian-facing ბრაუზერული click-through ვერ შესრულდა ამ Windows სესიაში ვიზუალურად (ამ გარემოში browser-ავტომატიზაციის ხელსაწყო (Playwright/chromium-cli) დაყენებული არ არის) — მაგრამ `PickupConsentWorkflowTest`-ის 10 ტესტი ზუსტად იმავე HTTP-ციკლს ფარავს რეალურ request/response-ებით (guardian ამატებს/შლის authorized pickup-ს, admin აქვეყნებს consent ფორმას, guardian პასუხობს/ცვლის პასუხს დუბლიკატის გარეშე, cross-tenant/cross-guardian/revoked-link/`can_pickup=false` ყველა უარყოფილია), რაც ფუნქციურად იგივე დადასტურებას იძლევა, რაც ვიზუალური click-through.

**გასწორებული ფაქტობრივი ხარვეზი**: `docs/design-parity-checklist.md`-ის "Safety (pickup/consent)" მწკრივს ადრე ჰქონდა `verified_local` **+ production** ლეიბლი — ეს ზედმეტად ძლიერი მტკიცება იყო, რადგან production-ზე რეალურად მხოლოდ route-რეგისტრაცია და migration-status დადასტურდა, არა guardian-ის მხრიდან ფაქტობრივი გამოყენება. გასწორდა `verified_local`-ზე (production-ის deploy-ისტორია დარჩა აღწერაში, მაგრამ "+ production" claim მოიხსნა) — იგივე პატერნი, რაც უკვე გამოყენებული იყო Admissions CRM-ის მწკრივში ქვემოთ.

**რჩება ღია, მომხმარებლის გადაწყვეტილებას საჭიროებს** (არ არის ავტომატურად ამოხსნადი კოდის ცვლილებით): (ა) რეალური მშობლის/მოსწავლის ონბორდინგი სკოლის საკუთარი პროცესით, ან (ბ) ცხადი ავტორიზაცია `admin:reset-password`-ის გასაშვებად production-ზე, რომ ერთხელ მაინც შესრულდეს admin-მხრიდან publish + roster-ნახვა (guardian-მხრიდან respond მაინც დარჩება დაუდასტურებელი, სანამ რეალური მშობელი არ გაჩნდება). ვინაიდან ეს სესია არცერთი კოდის ცვლილება არ შეიცავს (მხოლოდ დოკუმენტაციის ორი ზუსტება), ახალი production დეპლოი არ შესრულებულა — ცარიელი/no-op დეპლოი "ბრმა გამეორება" იქნებოდა, რასაც დავალება პირდაპირ კრძალავს.

### 2026-09-15 (გაგრძელება, მე-2) — Admissions CRM production ვერიფიკაცია: infra blocker დახურულია (`admin:reset-password`), დოკუმენტაციის ხარვეზები გასწორებული

Admissions CRM-ის (`26cd1d6` — lead stage tracking, `admission_appointments`, `admission_decisions`) production ვერიფიკაცია დაიწყო ცალკე სესიაში იმავე ავთენტიფიცირებული click-through-ის (kanban სია → lead → stage → ვიზიტი → გადაწყვეტილება) მოთხოვნით. **ყველა შემოწმება, რომელიც არ საჭიროებდა production-ზე ავთენტიფიცირებულ ბრაუზერულ სესიას, სუფთაა** — დამოწმებულია ამ სესიაშიც, დამოუკიდებლად, read-only SSH/`curl`-ით: ორივე ახალი migration (`2026_09_23_000001`/`_000002`) `migrate:status`-ში `Ran`-ია (batch 11); ყველა 5 ახალი route რეგისტრირებულია (`route:list --path=admissions`), `/portal/admissions-pipeline` production-ზე `302`-ს აბრუნებს არაავტორიზებული სტუმრისთვის (სწორი); აშენებული frontend asset (`admissions-pipeline-DApWnFMk.js`) რეალურად არსებობს production `public/build/assets/`-ში; `AdmissionsPipelineTest` (4/4, 72 assertion) ლოკალურად ფარავს სრულ workflow-ს (public lead capture → ყველა stage-ის დაწინაურება → ვიზიტის დაჯავშნა → გადაწყვეტილება, აუდიტ-ჩანაწერებით) და forward-only stage-გადასვლის უარყოფას, მასწავლებლის 403-ს, cross-tenant იზოლაციას; სრული suite (228/228, 1117 assertion — იხ. ქვემოთ) რეგრესიას არ აჩვენებს.

**ზუსტად იგივე ცნობილი, ინფრასტრუქტურული ბლოკერი**, რაც Assignments-ისთვის (2026-09-14) და Substitution-ისთვის (2026-09-15) იყო დოკუმენტირებული ზემოთ: production admin-ის (`admin@aisi.edu.ge`) ერთჯერადი seed-პაროლი დაკარგულია, mailbox-წვდომის-გარეშე password-reset ნაკადი არ არსებობს, session store Redis-ია (არა წასაკითხი/ფალშივად-შესაქმნელი DB ცხრილი), და impersonation ფუნქცია საერთოდ არ არსებობს კოდში — ანუ production-ზე ავთენტიფიცირებული admin/director/academic_manager სესიის მიღება ობიექტურად შეუძლებელი იყო ამ ორი წინა ვერიფიკაციის დროსაც. დამატებით, production-ს ჯერ 0 `admission_leads`/`admission_appointments`/`admission_decisions` აქვს (ორგანულად — არცერთ რეალურ მშობელს ჯერ არ შემოუტანია საჯარო ფორმა ამ pipeline-ის გაშვების შემდეგ), ანუ სესიის მოპოვების შემდეგაც კლიკ-გავლა რეალურ lead-ზე ჯერ არ იქნებოდა შესაძლებელი — ეს ცალკე, ჩვენგან დამოუკიდებელი ფაქტია (საჯარო ფორმის ხელოვნურად შევსება production-ზე ტესტისთვის CLAUDE.md-ის დემო/production გამიჯვნის წესს დაარღვევდა).

**გასწორება (root cause, არა კვლავ-ცდა)**: ორივე წინა ვერიფიკაციამ ერთსა და იმავე რეკომენდაციაზე მიუთითა — `php artisan tinker`-ით პაროლის ხელით გადაწერის ნაცვლად საჭიროა დოკუმენტირებული, აუდიტირებადი Artisan command. დაემატა `admin:reset-password {email} {--tenant=} {--reason=} {--force}` (`app/Console/Commands/ResetUserPassword.php`):

- მოითხოვს ცხად `--tenant`-ს და ამოწმებს, რომ მითითებულ email-ს ჰქონდეს **აქტიური** `TenantMembership` ამ tenant-ში — არასოდეს არ წყვეტს პაროლს ამ კონტექსტის გარეშე.
- მოითხოვს ცხად `--reason`-ს (მინიმუმ 10 სიმბოლო) — ეს ერთადერთი ჩანაწერია მიზეზის შესახებ, რადგან თვითონ ახალი პაროლი არასდროს ილოგება.
- ინტერაქტიულ დასტურს ითხოვს (`--force`-ით გამოტოვებადი) და `AuditLogger`-ში წერს `user.password_reset_via_cli` მოვლენას (CLAUDE.md ინვარიანტი #7) — `meta`-ში მხოლოდ `reason`/როლები/command name, **არასდროს პაროლი** (ტესტით დამოწმებული: `assertArrayNotHasKey('password', ...)` და serialized meta-ში ახალი პაროლი არ გვხვდება).
- ახალი პაროლი (`Str::password(24)`) console-ში ჩანს **ერთხელ**, არასდროს ინახება plaintext-ად არცერთ ადგილას (log/audit/DB — `password` cast ჰეშავს save-ისას).
- `remember_token` ხელახლა გენერირდება (ძველი "დამახსოვრებული" cookie-ები აღარ მუშაობს).

7 ახალი ტესტი (`tests/Feature/Console/ResetUserPasswordCommandTest.php`): წარმატებული reset + ერთი აუდიტ-ჩანაწერი პაროლის-გარეშე meta-თი, ახალი პაროლი რეალურად ცვლის hash-ს (ძველი factory-პაროლი აღარ მუშაობს), უარყოფა `--tenant`-ის გარეშე, უარყოფა `--reason`-ის გარეშე, უარყოფა გაუქმებული (`is_active=false`) წევრობისთვის, უარყოფა უცნობი tenant slug-ისთვის, დასტურის უარყოფაზე (`no`) არაფერი არ იცვლება. ეს command **წვდომას არ ცვლის** — მხოლოდ პაროლს; MFA/როლის შემოწმება login-ის დროს უცვლელია.

**ცალკე დახურული ხარვეზი (დოკუმენტაცია)**: `docs/design-parity-checklist.md`-ში "Admissions CRM" მწკრივი `not_started`-ად იყო მონიშნული `26cd1d6`-ის შემდეგაც — განახლდა `verified_local`-ზე ზემოთ აღწერილი დეტალებით (Substitute-მწკრივის იგივე ფორმატით). ასევე, `dea9814`-ის (Assignments) მსგავსად, `26cd1d6`-ის (Admissions pipeline) production-დეპლოიც ფაქტობრივად შესრულებული იყო, მაგრამ არასდროს ჰქონია ცალკე "deploy evidence" commit — ეს ჩანაწერი ავსებს იმ ხარვეზსაც: ზემოთ ჩამოთვლილი read-only შემოწმებები (route/migration/asset/302) ამ სესიაში დამოუკიდებლად, ხელახლა დამოწმებულია production-ზე, არა მხოლოდ წინა სესიის ანგარიშზე დაყრდნობით.

**ტესტები ამ ცვლილების შემდეგ**: `php artisan test` → **228/228 passed, 1117 assertions**; Pint სუფთაა; PHPStan (level 7) → 0 error; `npm run types:check` → 0 error; `npm run build` → წარმატებული.

**Production deploy — 2026-09-15**: commit `2f8168c` (`admin:reset-password`-ის ერთადერთი ახალი ფაილი — migration/route/frontend ცვლილება არ იყო საჭირო) რეალურად დეპლოირებულია იმავე დადგენილი ნიმუშით: `git diff --name-only 26cd1d6..HEAD -- app database resources routes` სიით ერთი შეცვლილი ფაილი + მთლიანი ახალი `public/build` → `scp` → `tar -xzf` → `clpctl system:permissions:reset --path=. --directories=770 --files=660` → `storage/framework/views/*.php` წაშლა → `migrate --force` ("Nothing to migrate", schema უცვლელი) → `route:clear && route:cache` → `config:clear && config:cache`.

დამოწმებულია production-ზე რეალურად: `php artisan list admin` აჩვენებს ახალ `admin:reset-password` command-ს რეგისტრირებულს (ერთადერთი ამ namespace-ში); `migrate:status`-ში ორივე Admissions migration კვლავ `Ran`-ია, უცვლელი (ეს იყო code-only დეპლოი). Smoke checks: `/`, `/about`, `/learning`, `/school-life`, `/news`, `/library`, `/contact`, `/login`, `/robots.txt`, `/sitemap.xml`, `/teachers` → ყველა 200; `/portal/admissions-pipeline`, `/portal/timetable`, `/portal/substitutions`, `/portal/academic-structure`, `/portal/members`, `/portal/assignments` → ყველა 302 → `/login` (სწორი, არაავტორიზებული სტუმრისთვის). Production `storage/logs/laravel.log`-ის ბოლო ჩანაწერები ამ დეპლოის დროსაც/შემდეგაც — ყველა ძველი, უკვე ცნობილი/გასწორებული ჩანაწერია (2026-09-10/2026-09-14); ახალი შეცდომა არცერთი არ დამატებულა. დეპლოის დროებითი tar.gz სერვერიდან და ლოკალურად წაშლილია.

**კვლავ ღია დარჩენილი** (ინფრასტრუქტურული, არა კოდის ხარვეზი — ახლა უკვე *მოგვარებადი*, არა მუდმივად ბლოკირებული): production-ზე ავთენტიფიცირებული click-through ჯერ არ შესრულებულა, რადგან ეს ცალკე, ცხადი production-write ქმედებაა (რეალური `admin@aisi.edu.ge`-ის პაროლის შეცვლა) — ეს ამ ფიქსის scope-ის მიღმაა და მოითხოვს მომხმარებლის ცხად თანხმობას, სანამ ვინმე ამ ახალი command-ით რეალურად შეცვლის production admin-ის წვდომას. ასევე, production-ს ჯერ არცერთი რეალური admission lead არ აქვს (იხ. ზემოთ) — pipeline-ის კლიკ-გავლა საჭიროებს ან რეალურ მშობლის submission-ს, ან ადამიანის ხელით ტესტს რეალურ admin ანგარიშზე.

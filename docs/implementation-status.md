...
**Production deploy**: ეს ჩანაწერი ჯერ მხოლოდ ლოკალურ ცვლილებას აღწერს (commit-ის და deploy-ის დეტალები ცალკე commit-ში დაემატება ამ commit-ის შემდეგ, იმავე დადგენილი deploy-ნიმუშით: `npm run build` → შეცვლილი `app`/`database`/`resources`/`routes` ფაილები + ახალი `public/build` → `scp` → `tar -xzf` → `clpctl system:permissions:reset` → `migrate --force` → `route:clear && route:cache` → `config:clear && config:cache`).

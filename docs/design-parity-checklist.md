...
| Safety (pickup/consent) | `/portal/pickup`, `/portal/consents` | `verified_local` — authorized-pickup list gated on `guardian_links.can_pickup` (`app/Domain/PickupConsent`); consent forms publish→guardian responds (accept/decline) workflow, admin/director-only publish, teacher class-roster view. 10 tests, full local suite 238/238 (1168 assertions) with Pint/PHPStan level 7/`tsc --noEmit` all clean. Deployed to production 2026-09-15 (commit `ba30bb2`, see `docs/implementation-status.md`); 3 migrations run on production MySQL; new routes were invisible until `route:clear && route:cache` (same stale `bootstrap/cache/routes-v7.php` gotcha documented 2026-09-14) — after that fix all 7 `pickup.*`/`consents.*` routes and the homepage returned correct HTTP codes on production. **Corrected 2026-09-15 (independent re-verification): not `verified_production`** — the earlier "+ production" label overstated what was actually confirmed. Production has 0 guardians and 0 students (only the seeded admin account exists), so the guardian-facing half of this workflow (the `pickup` nav item rendering, adding/removing a pickup person, responding to a consent form) has never been observed live and cannot be until a real guardian is onboarded or an authorized admin-password reset opens a session — same pre-existing data/credential gap already logged honestly for Admissions CRM below. Route registration, migration state, and code-level tenant/guardian/`can_pickup` scoping were independently re-reviewed and are correct; no code, migration, or deploy-step defect was found. |
...
| Admissions CRM | `/portal/admissions-pipeline` | `verified_local` — lead now tracks `stage` (new→contacted→visit_scheduled→documents_submitted→decided, forward-only), `admission_appointments` (scheduled visits) and `admission_decisions` (accepted/declined/waitlisted, audit-logged). Kanban list + lead-detail screens, admin/director/academic_manager only, tenant-isolated. 4 tests, 72 assertions — full real-HTTP path (public lead capture → advance every stage → schedule appointment → record decision), forward-only rejection, teacher-forbidden, cross-tenant isolation. Deployed to production 2026-09-15 (commit `26cd1d6`, see `docs/implementation-status.md`); **not yet `verified_production`** — production has 0 real leads yet (no prospective parent has submitted the public form since this pipeline went live) and no session could be obtained to click through as admin/director/academic_manager (same pre-existing admin-password-recovery infra gap as Substitute/Assignments, now closed by `admin:reset-password`, see implementation-status). |

...
| Safety (pickup/consent) | `/portal/pickup`, `/portal/consents` | `verified_local` + **production** — authorized-pickup list gated on `guardian_links.can_pickup` (`app/Domain/PickupConsent`); consent forms publish→guardian responds (accept/decline) workflow, admin/director-only publish, teacher class-roster view. 10 tests. Deployed to production 2026-09-15 (commit `ba30bb2`, see `docs/implementation-status.md`); 3 migrations run on production MySQL; new routes were invisible until `route:clear && route:cache` (same stale `bootstrap/cache/routes-v7.php` gotcha documented 2026-09-14 — recurred because it silently regenerates after any prior deploy that doesn't run it; now confirmed still required on **every** deploy, not a one-off) — after that fix all 7 `pickup.*`/`consents.*` routes and the homepage returned correct HTTP codes on production. |
